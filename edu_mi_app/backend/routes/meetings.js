            ...
            if (schedule && schedule.is_active) {
                ...
                const { data: newMeeting, error: createError } = await supabase
                    ...
                    .insert({
                        ...
                        allowed_users: schedule.allowed_users || [],
                        meeting_type: schedule.schedule_type || 'master',
                        is_virtual: true
                    })
                    .select()
                    .single();
...
router.post('/:meetingId/user-left', async (req, res, next) => {
    try {
        ...
        if (!remoteUid) {
            return res.status(400).json({
                error: { message: 'remoteUid es requerido en el cuerpo' }
            });
        }
 ...
 * Limpia participantes inactivos (sin heartbeat en los últimos 6 segundos)
...
router.post('/cleanup-inactive', authenticateUser, async (req, res, next) => {
    try {
        const inactiveThreshold = new Date(Date.now() - 180 * 1000); // 3 minutos atrás
    ...
    try {
        ...
        const { data: schedule, error } = await supabase
            ...
            .insert({
                ...
                is_active: true,
                schedule_type: req.body.schedule_type || 'master',
...
router.get('/:meetingId/students-status', authenticateUser, async (req, res, next) => {
    try {
        ...
        // 1. Obtener la reunión y sus restricciones
        const { data: meeting, error: meetingError } = await supabase
            .from('meetings')
        ...
        const allowedGroups = meeting.allowed_groups || [];
        const allowedUsers = meeting.allowed_users || [];

        // 2. Obtener participantes actuales (los que ya se unieron o están esperando)
...
${JSON.stringify(participants, null, 2)}`);
        ...
        // Agregar IDs de estudiantes esperados por grupo
        let finalGroups = [...allowedGroups];
        if (finalGroups.length === 0) {
            // Si no hay grupos, intentar usar el del creador como fallback para poblar la lista de "Ausentes"
            const { data: creator } = await supabase
                .from('profiles')
                .select('group_name')
                .eq('user_id', meeting.creator_id)
                .single();
            if (creator && creator.group_name) {
                finalGroups = [creator.group_name];
            }
        }
        ...
        if (finalGroups.length > 0) {
            const { data: groupStudents } = await supabase
                .from('profiles')
                .select('user_id')
                .in('group_name', finalGroups)
                .eq('role', 'student');
            if (groupStudents) groupStudents.forEach(s => studentIdsSet.add(s.user_id));
        }

        // Agregar IDs de usuarios específicos permitidos
        if (allowedUsers.length > 0) {
            allowedUsers.forEach(id => studentIdsSet.add(id));
        }

        // 4. Obtener perfiles completos de todos los IDs identificados (solo si son estudiantes)
        ...
        const { data: studentProfiles, error: profilesError } = await supabase
            .from('profiles')
            .select('user_id, full_name, role')
            .in('user_id', Array.from(studentIdsSet))
            .eq('role', 'student');

        if (profilesError) throw profilesError;

        // 5. Mapear estado de cada estudiante
