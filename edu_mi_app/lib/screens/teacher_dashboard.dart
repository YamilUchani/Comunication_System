  ...
  void _showAssignAchievementsDialog(
    ...
    try {
      ...
      showDialog(
        ...
        builder: (context) => StatefulBuilder(
          builder: (context, setState) => AlertDialog(
            title: Text('Asignar Logros a $studentName'),
            content: SizedBox(
              width: double.maxFinite,
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Text('Selecciona los logros para otorgar hoy:'),
                  const SizedBox(height: 10),
                  Expanded(
                    child: ListView.builder(
                      shrinkWrap: true,
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isSelected = selectedAchievements.contains(
                          achievement['id'],
                        );
                ...
                onPressed: () async {
                  try {
                    await ApiService.assignAchievementsToAttendance(
                      attendanceId,
                      selectedAchievements.toList(),
                    );
                    if (mounted) {
                      Navigator.pop(context);
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Logros asignados exitosamente'),
                        ),
                      );
                    }
                  } catch (e) {
